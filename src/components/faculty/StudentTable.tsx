...
export default function StudentTable() {
  ...
  // Function to handle selecting/deselecting all students
  const handleSelectAll = () => {
    if (apiData?.enrolled_students) {
      if (selectedStudents.length === apiData.enrolled_students.length) {
    ...
    try {
      ...
      response.data.results.forEach((obj) => {
        if (obj.success === false) {
          setStudentsNotSubmitted((prev) => [...prev, obj.student_id]);
        }
  ...
  return (
    <>
      <div className="px-4 py-8 max-w-7xl mx-auto">
        ...
        {apiCalled && apiData?.enrolled_students && (
          <div
          ...
          >
            <span className="block sm:inline">
              {" "}
              {`Error submitting request to ${
                apiData.enrolled_students.length - submitted
              } students. Please check if you are not sending request to same student again`}
            </span>
          </div>
        )}
                          ...
                          <input
                            ...
                            checked={
                              ...
                              selectedStudents.length ===
                                apiData.enrolled_students.length
                            }
